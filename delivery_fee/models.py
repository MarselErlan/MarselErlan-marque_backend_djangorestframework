"""
Delivery Fee models for marketplace delivery fee management.
...
class DeliveryFee(models.Model):
    """Delivery fee configuration based on product category structure.
    ...
    second_subcategory = models.ForeignKey(
        'products.Subcategory',
        on_delete=models.CASCADE,
        related_name='second_level_delivery_fees',
        null=True,
        blank=True,
    ...
    description = models.TextField(
        null=True,
        blank=True,
        help_text="Description of this delivery fee configuration"
    )
    ...
    def __str__(self):
        ...
        return f"{' -> '.join(parts)}: {self.fee_amount}"
    
    def clean(self):
        """Validate fee configuration"""
        errors = {}
        
        # Category is required (check if it's set, not if it exists)
        if not self.category_id:
            errors['category'] = "Category is required."
            # If category is missing, we can't validate other fields
            if errors:
    ...
    def get_fee_for_product(cls, product):
        """Get the most specific delivery fee for a product.
        ...
        Returns:
            DeliveryFee instance or None if no matching fee found
        """
